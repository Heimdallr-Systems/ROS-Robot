...
namespace Codegen
{
  namespace coder
  {
    ...
    static void b_computeScaleFactors(const double xv[3], const double yv[3],
      int nloops, const int first[3], const int last[3], double scale[3]);
    static void b_countLoops(const double xv[3], const double yv[3], int *nloops,
      int first[3], int last[3]);
    static void b_findLast(int *k, const double xv[3], const double yv[3], int
      nx);
    static bool b_scalarInpolygon(double xj, double yj, const double xv[3],
      const double yv[3], int nloops, const int first[3], const int last[3],
      double minxv, double maxxv, double minyv, double maxyv, const double
      scale[3]);
    static void b_skipNaNs(int *k, const double xv[3], int nx);
    static void computeRange(const double xv[4], int nloops, const int first[4],
      const int last[4], double *minxv, double *maxxv);
    static void computeScaleFactors(const double xv[4], const double yv[4], int
      nloops, const int first[4], const int last[4], double scale[4]);
    static void contrib(double x1, double b_y1, double x2, double y2, signed
                        char quad1, signed char quad2, double scale, signed char
                        *diffQuad, bool *onj);
    static void countLoops(const double xv[4], const double yv[4], int *nloops,
      int first[4], int last[4]);
    static void findLast(int *k, const double xv[4], const double yv[4], int nx);
    static bool scalarInpolygon(double xj, double yj, const double xv[4], const
      double yv[4], int nloops, const int first[4], const int last[4], double
      minxv, double maxxv, double minyv, double maxyv, const double scale[4]);
    static double scalemax(double a, double b);
    static void skipNaNs(int *k, const double xv[4], int nx);
  }
}
  ...
  namespace coder
  {
      ...
      int nloops, const int first[3], const int last[3], double scale[3])
    {
      ...
      for (int j = 0; j < nloops; j++) {
        ...
        for (int b_i = i; b_i <= i1; b_i++) {
          scale[b_i - 1] = scalemax(b_abs(0.5 * (xv[b_i - 1] + xv[b_i])), b_abs
            (0.5 * (yv[b_i - 1] + yv[b_i]))) * 6.6613381477509392E-16;
        }
        ...
        i2 = first[j];
        i3 = last[j];
        scale[i3 - 1] = scalemax(b_abs(0.5 * (xv[i3 - 1] + xv[i2 - 1])), b_abs
          (0.5 * (yv[i3 - 1] + yv[i2 - 1]))) * 6.6613381477509392E-16;
      }
    }
    ...
    bool b_inpolygon(double x, double y, const double xv[3], const double yv[3])
    {
      ...
      if (nloops != 0) {
        ...
        b_computeScaleFactors(xv, yv, nloops, first, last, b_dv);
    ...
    bool inpolygon(double x, double y, const double xv[4], const double yv[4])
    {
      ...
      if (nloops != 0) {
        ...
        computeScaleFactors(xv, yv, nloops, first, last, b_dv);
        in = scalarInpolygon(x, y, xv, yv, nloops, first, last, minxv, maxxv,
                             minyv, maxyv, b_dv);
      }
