...
namespace Codegen
{
    ...
    legs_on_gnd[4], double Theta1[4], double Theta2[4], double Theta3[4])
  {
    ...
    while (!loop_toggle) {
      std::cout << "BPC: inside !loop_toggle loop\n";
      //     %% FR LEG
      loop_toggle = true;
      if ((static_cast<int>(legs_on_gnd[0])) == 1) {
        ...
        std::cout << "BPC: Checking for D_FR\n";
